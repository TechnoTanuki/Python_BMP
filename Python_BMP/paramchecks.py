...
from .bmpconstants import bmpcolorbits, bmpx, bmpy
from .inttools import readint
from .primitives2D import entirecircleisinboundary

f = lambda bmp, x, y: (x<readint(bmpx,4,bmp) and y<readint(bmpy,4,bmp)) and (x>-1 and y>-1)

def intcircleparam(func):
    """Decorator to test if the 2nd,3rd,4th parameters in a function
...
def func24bitonlyandentirecircleinboundary(func):
    """Decorator to restrict the use of this function
        ...
        x,y and radius of a circle that lay within the RGB bitmap.
    ...
    def callf(*args, **kwargs):
        ...
        if bmp[bmpcolorbits] != 24: 
            ...
            if (type(x)==int and type(y)==int) and type(r)==int:
                if entirecircleisinboundary(x,y,-1,readint(bmpx,4,bmp),-1,readint(bmpy,4,bmp),r): 
                    return(func(*args, **kwargs))
                else: 
                    print(sysmsg['regionoutofbounds'])
            else:  
...
def func8and24bitonlyandentirecircleinboundary(func):
    """Decorator to restrict the use of this function 
        ...
        and ensure that the 2nd, 3rd, 4th parameters are ints 
        whose values when interpreted as  
        x,y and radius of a circle that lay within the RGB bitmap.
...
def func8and24bitonly(func):
    """Decorator to restrict the use of this function to only 
        24-bit or 8-bit bitmaps (1st parameter)
...
def func8and24bitonlyandentirerectinboundary(func):
    """Decorator to restrict the use of this function
        to only 24 bit or 8 bit bitmaps (1st parameter) 
        and ensure that the 2nd, 3rd, 4th and 5th parameters are ints
        whose values when interpreted  as x and y coordinates 
        of a rectangle that lay within the RGB bitmap.
...
def entirecircleinboundary(func):
    """Decorator to ensure that the 2nd, 3rd, 4th parameters are ints
        whose values when interpreted
        as the centerpoint x,y and radius r of a circle
    ...
    def callf(*args, **kwargs):
        ...
        r = args[3]
        if (type(x)==int and type(y)==int) and type(r)==int:
            if entirecircleisinboundary(x,y,-1,readint(bmpx,4,bmp),-1,readint(bmpx,4,bmp),r):
                return(func(*args, **kwargs))
            else: 
                print(sysmsg['regionoutofbounds'])
        else:  
