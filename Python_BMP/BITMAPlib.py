...
from .colors import(
    ...
    int2RGBarr,
    int2RGBlist,
    invertbitsinbuffer as _invbtbf,
    isvalidcolorbit,
    makeBGRbuf as _mkBGRbf,
    matchRGBtopal,
    monochrome,
...
def fliphoricircregion(bmp: array,
        x: int, y: int, r: int):
    """Flips horizontally a
        circular region with a
        center at (x, y) and a radius r
...
def flipvertcircregion(bmp: array,
        x: int, y: int, r: int):
    """Does a vertical flip of a
        circular area defined by
        centerpoint (x,y) and radius r
...
def horibrightnessgrad2circregion(
    ...
    """Applies a horizontal brightness
        gradient adjustment to a
        circular region with a center
        at (x, y) and radius r
...
def colorfiltercircregion(bmp: array,
    ...
    """Applies a color filter defined
        by rgbfactors to a circular area
        defined by centerpoint (x, y)
        and radius r
...
def copyrect(bmp: array,
        x1: int, y1: int,
        x2: int, y2: int) -> array:
    """Copy a rectangular region to a
        custom buffer
...
def convertselection2BMP(buf: array):
    """Converts a custom unsigned
        byte array to the bmp format
...
def _usebyref24btfn2reg(bmp: array,
    ...
    """Apply byref func(funcparam) to
        a rectangular area in a 24-bit
        bitmap
...
def mirrorleftinregion2file(
    ...
    """Mirrors the left-half region
        in a rectangular area in a bmp
...
def showsimilarparts(
        ...
        inputfile2: str,
        diff_file: str):
    """Compares 2 files and saves
        the similar parts to a
        bitmap file
...
def monochromecircregion2file(
        ...
        NewBMPfile: str,
        x: int, y: int, r: int):
    """Applies a monochrome filter to a
        circular region
...
def colorfiltercircregion2file(
        ...
        x: int, y: int, r: int,
        rgbfactors: list[float, float, float]):
    """Applies a color filter to a
        circular area in a bitmap file
...
def sphere2file(ExistingBMPfile: str,
        ...
        x: int, y: int, r: int,
        rgbfactors: list[float, float, float]):
    """Renders a sphere in a circular
        region in a bitmap file
        with color set by rgbfactors
...
def filledcircle2file(
        ...
        x: int, y: int, r: int,
        color: int):
    """Draws a filled circle in a bmp
...
def magnifyNtimescircregion2file(
        ...
        x: int, y: int, r: int,
        intmagfactor: int):
    """Magnify a circular region by an
        integer factor n times
...
def pixelizenxncircregion2file(
        ...
        x: int, y: int, r: int,
        intpixsize: int):
    """Apply a pixel blur by averaging
        n by n rectangular regions
        a circular area
...
def copycircregion2file(
    ...
    """Does a copy/paste of a circular
        region in a bitmap file
