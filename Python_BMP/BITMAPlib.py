...
def compute8bitBMPoffset(bmp: array,
    ...
    """
    return x + ((_rdint(_bmy, 4, bmp) - y - 1) * \
...
def compute1bitBMPoffsetwithheader(
    ...
    return (x >> 3) + \
           ((_rdint(_bmy, 4, bmp) - y - 1) * \
            computexbytes(_rdint(_bmx, 4, bmp), 1)) + 62
...
def plotRGBxybit(bmp: array,
    ...
    if isinBMPrectbnd(bmp,x,y):
        if bmp[_bmclrbits] == 24:
            ...
            plotxybit(bmp,
                x, y,
                matchRGBtopal(rgb, getallRGBpal(bmp)))
...
def swapcolors(bmp: array,
        p1: list, p2: list):
    """Swaps the colors of two points
        in a bitmap 
...
def itercopyrect(bmp: array,
    ...
    """
    x1, y1, x2, y2=sortrecpoints(x1, y1, x2, y2)
    bufsize = adjustbufsize(x2 - x1 +1, bmp[28])
    r = getxcharcount(bmp)
...
def drawvec(bmp: array,
    ...
    def hdaddvect(bmp, v, hm, a1, a2):
        linevec(bmp, v,
            addvect(v, polar2rectcoord2D([hm, a1])), color)
        linevec(bmp, v,
            addvect(v, polar2rectcoord2D([hm, a2])), color)
    def hdsubvect(bmp, v, hm, a1, a2):
        linevec(bmp, v,
            subvect(v, polar2rectcoord2D([hm, a1])), color)
        linevec(bmp, v,
            subvect(v, polar2rectcoord2D([hm, a2])), color)
...
def imgregionbyRGB2file(
        ...
        rgb: list, similaritythreshold: float,
        showedgeonly:bool):
...
def colorfilter2file(ExistingBMPfile: str, NewBMPfile: str,
    ...
    """
    applycoloradjfunc(ExistingBMPfile, NewBMPfile,
...
def upgradeto24bitimage2file(
    ...
    """Upgrades a bitmap file 
        (1,4,8 bits color depth) to 24-bits
    ...
    """
    applyfuncandsave(ExistingBMPfile, NewBMPfile,
        upgradeto24bitimage)
